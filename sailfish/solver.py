...
class Scheme:
    ...
    def __init__(self, config: Sailfish):
        ...
        define_macros["ANG_CONS"] = int(config.scheme.angular_momentum_fix)
    ...
    '''
    Lz_godenov_flux_code  = R"""

    """

    @kernel(code = Lz_godenov_flux_code)
    def Lz_godenov_flux():
    '''
        ...
        int nk)
    {
        ...
        //0. l0 = angmom(u0,xv)
        //2. l1 = angmom(u1,xv)
        //2. u2=rk*u0+(1-rk)*(u1+du)
        //3. l2=rk*l0+(1-rk)*(l1+dl(f,xv))
        //4. u2 = fixup(u2,l2,xv)
        //5. write u2 to mem
        ...
        #endif
        {
            ...
            #if ANG_CONS==1
            double xvm = xv[0 * nd / nq + nccc / sf];
            double xvp = xv[0 * nd / nq + nrcc / sf];
            double yvm = xv[1 * nd / nq + nccc / sf];
            double yvp = xv[1 * nd / nq + ncrc / sf];
            double xc = 0.5 * (xvm + xvp);
            double yc = 0.5 * (yvm + yvp);
            l0 = xc * urk[nccc + 2 * sq] - yc * urk[nccc + 1 * sq];
            l1 = xc * uc[2 * sq] - yc * uc[1 * sq];
            #endif
    ...
    def update_cons_from_fluxes(
        ...
        dim = self._dim
        s = u.shape[:3]
        return s[:dim], (urk, u, f, stm, dv, xv, dt, rk, *s)

    '''
    update_Lz_from_fluxes_code = R"""
    KERNEL void  update_Lz_from_fluxes_code(
        double *lrk,
        double *l,
        double *f,
        double *stm,
    ...
    @kernel(code = update_Lz_from_fluxes_code)
    def update_LZ_from_fluxes(
        self,
        lrk,
    ...
    while True:
        ...
        for rk in rks or [0.0]:
            ...
            if cache_flux:
                ...
                update_cons_from_fluxes(u0, u1, fh, stm, dv, xv, dt, rk)
            else:
